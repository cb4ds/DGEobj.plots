...
#' @param symbolShape Shape of the symbols for uSignificant and Not Significant points (default = c('circle', 'circle')
...
cdfPlot <- function(contrastDF,
    ...
    contrastDF_subset <- contrastDF %>%
        filter(!!rlang::sym(pvalCol) <= pvalMax)
    cdfMain <- NULL
    cdfInset <- NULL
