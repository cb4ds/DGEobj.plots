...
#get transcript level data and keep max for each gene
# Now get the transcript IDs and lengths from Ensembl/biomaRt
ens.ds      <- "rnorvegicus_gene_ensembl"
ens.mart    <- useMart(biomart = "ensembl", dataset = ens.ds)
...
transcript.data <- getBM(attributes = ens.columns,
                         ...
                         mart = ens.mart)
# Keep one row per gene with max(transcript_length)
transcript.data %<>%
